...
int8_t XDualDCMotor::onAccess(uint8_t api, const uint8_t *param, uint8_t psize, uint8_t *result, uint8_t *rsize)
{
	LOGN("XDualDCMotor::onAccess(");LOG(api);LOGN(")");
	(void)psize; (void)result;

    if (api == XDualDCMotor_API_setMotorSpeed) {
		uint8_t motor;
		int8_t speed;
		param = fetchU8(param, &motor);
		param = fetchU8(param, (uint8_t *)&speed);
		setMotorSpeed(motor, speed);
		*rsize = 0;
	}
	else if(api == XDualDCMotor_API_setAllSpeed) {
		int8_t speed1, speed2;
		param = fetchU8(param, (uint8_t *)&speed1);
		param = fetchU8(param, (uint8_t *)&speed2);
		setAllSpeed(speed1, speed2);
		*rsize = 0;
	}
	else if(api == XDualDCMotor_API_stopMotor) {
		uint8_t motor;
		param = fetchU8(param, &motor);
		stopMotor(motor);
		*rsize = 0;
	}
	else if(api == XDualDCMotor_API_stopAllMotor) {
		stopAllMotor();
		*rsize = 0;
	}
	else {
